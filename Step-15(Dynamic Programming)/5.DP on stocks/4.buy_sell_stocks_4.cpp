/*
EXAMPLE:
Input: prices = {3, 3, 5, 0, 0, 3, 1, 4}, n = 8, k = 2
Output: 6
...
QUESTION:
Given an array `prices` where prices[i] is the price of a given stock on the ith day, design an algorithm to find the maximum profit. You may complete at most `k` transactions. You may not engage in multiple transactions at the same time (i.e., you must sell the stock before you buy again).

APPROACH:
1. Use dynamic programming to keep track of the maximum profit achievable at each day with at most `k` transactions.
...
int maxProfitTabulation(vector<int>& Arr, int n, int k) {
    ...
    return dp[0][0][k]; // The result is stored in dp[0][0][k] which represents maximum profit after the final transaction.
}
...
/*
TIME COMPLEXITY:
- All approaches: O(n * k), where n is the number of elements in the array and k is the maximum number of transactions. This is because we iterate through the array and transactions multiple times.

SPACE COMPLEXITY:
- Memoization approach: O(n * k) for the dp array.
- Tabulation approach: O(n * k) for the dp array.
- Space-optimized approach: O(k) for the cur and ahead arrays.

EXAMPLE USAGE:
vector<int> prices = {3, 3, 5, 0, 0, 3, 1, 4};
int n = prices.size();
int k = 2;
int result = maxProfit(prices, n, k); // Using space optimization approach
